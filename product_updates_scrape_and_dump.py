...
dt = datetime.now()
new_string = '''dt, "" '''
for i in range(len(list(db.columns))-2):
  new_string = new_string + ",dict({})"
new_string = "db.loc[len(db)] = [" + new_string + "]"
exec(new_string)
...
for i in range(len(data["name"])):
  ...
  db[data["name"][i]][ind]["apple"]=apple
  db[data["name"][i]][ind]["google"]=google

db.to_json('app updates repository .json')
